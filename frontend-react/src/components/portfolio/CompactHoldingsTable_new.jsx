...
const CompactHoldingsTable = ({ holdings = [] }) => {
  ...
  const processHoldings = (rawHoldings) => {
    ...
    if (firstItem.ticker && 
        (firstItem.current_value !== undefined || 
         firstItem.valor_atual !== undefined)) {
      return rawHoldings;
    }
    ...
    if (firstItem.symbol || firstItem.codigo) {
      return rawHoldings.map(item => ({
        ticker: item.symbol || item.codigo || item.ticker || 'N/A',
        ...
        is_us_ticker: item.is_us_ticker || item.is_us || item.isUS || false,
        is_bdr: item.is_bdr || item.isBDR || false
      }));
    }
  ...
  const sortedHoldings = processHoldings(holdings).sort((a, b) => 
  ...
  return (
    <Box
      sx={{
        position: 'fixed',
        right: 5, // 5 pixels do fim da página
        top: 80,
        width: '220px',
        ...
        <Table size="small" stickyHeader={false}>
          <TableHead>
            <TableRow sx={{ 
              ...
              <TableCell sx={{ width: '30%', fontWeight: 700, fontSize: '0.7rem' }}>Ativo</TableCell>
              <TableCell sx={{ ...cellStyles.numeric, width: '15%', fontWeight: 700, fontSize: '0.7rem' }}>Qtd</TableCell>
              <TableCell sx={{ ...cellStyles.monetary, width: '25%', fontWeight: 700, fontSize: '0.7rem' }}>P.Médio</TableCell>
              <TableCell sx={{ ...cellStyles.monetary, width: '30%', fontWeight: 700, fontSize: '0.7rem' }}>V.Atual</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {sortedHoldings.map((asset) => {
              ...
              return (
                ...
                >
                  ...
                  <TableCell sx={{ ...cellStyles.monetary, fontSize: '0.75rem' }}>
